...
- 지도 클릭으로 **내 위치(S)** 지정
- **화재 발생 방식 2가지**
  - *랜덤*: 화재 지점 수(1~5)만 정하면 무작위 발화. 시작할 때 내 위치도 무작위로 새로 선정
  - *직접 지정*: 실제 상황처럼 화재 위치를 지도 클릭으로 직접 지정 (다시 클릭 시 해제)

**2) 재생 단계 (자동 진행)**
- 시간 `t`가 **자동으로 0 → 1 → 2 …** 진행 (재생/일시정지/다음 단계/속도 조절)
- 화재가 매 시각 함께 확산
- **대피자(캐릭터)가 매 스텝 현재 화재 상황 기준으로 경로를 다시 잡으며(실시간 재경로) 한 칸씩 이동**해 비상구로 탈출
- **탈출 실패도 애니메이션**: 안전 경로가 사라지면 불을 피해 계속 이동하다 둘러싸이는 과정을 그대로 보여줌
- **탈출 성공 시**: 화재에 가려졌던 지나온 경로까지 지도에 드러내 표시
- 우측 패널: 진행 상태 + **전체 대피 통계**(랜덤 10명, 퀵정렬·이진탐색으로 목표 시간 내 탈출 인원 집계)
...
```
1. 사용자가 맵에서 현재 위치 클릭 → S 설정 / 화재 지정(랜덤·직접) → F 설정
2. BFS로 화재 확산 시뮬레이션 (t=0, 1, 2 ...)
3. 유니온-파인드로 S~X 연결 여부 실시간 확인
4. A*로 최단 탈출 경로 탐색 (도착 시점 기준 화재 차단)
5. 매 시각 t마다 현재 위치·현재 화재 기준으로 경로 재탐색하며 한 칸 이동
6. 안전 경로 소실 시 불을 피하는 best-effort 이동 → 둘러싸이면 실패
7. 결과 시각화 출력
```
...
| 노랑 | 비상구 (X) |
| 빨강 | 화재 셀 |
| 주황 | 현재 위치에서 출구까지 예정 경로 |
| 연보라 | 지나온 자취 (탈출 성공 시 화재 위에도 표시) |
| 파랑 | 엘리베이터 (E) |
| 회색 | 벽 (#) |

> 터미널 통합 테스트(`path_finder.visualize`)에서는 A* 탐색 노드(연파랑)도 함께 표시됩니다.
